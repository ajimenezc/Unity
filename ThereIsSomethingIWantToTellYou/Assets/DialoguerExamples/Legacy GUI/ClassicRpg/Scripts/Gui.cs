...
public class Gui : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		float rectY = (!_usingPositionRect) ? Screen.height - 240 : _positionRect.y;
		float rectWidth = (!_usingPositionRect) ? Screen.width : _positionRect.width;
		float rectHeight = (!_usingPositionRect) ? 480 : _positionRect.height;
		...
		if (_isBranchedText && _windowCurrentText == _windowTargetText && _branchedTextChoices != null)
		{
			...
			{
				float choiceRectY = (dialogueBoxRect.yMax - (((100) * _branchedTextChoices.Length) - (120 * i))) - 20;
				Rect choiceRect = new Rect(dialogueBoxRect.x + 60, choiceRectY, dialogueBoxRect.width - 80, 120);
